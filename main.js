...
const dataSetConfig = (esn = "", code = "", delimiter = "/") => {
  return [
    {
      name: "RSU_INFO",
      topic: topicReplace(`V2X.RSU.INFO.UP`, delimiter),
      description: "上报RSU信息",
    },
    {
      name: "RSU_MAP",
...
$(function () {
  ...
  $("#clientId").val(
    "RSU_" + (((1 + Math.random()) * 0x10000000) | 0).toString(16)
  );
  initTable();
});
...
function toggleConnect(event) {
  if (client) {
    ...
    client = mqtt.connect(
      `${protol}://${formData.host}:${formData.port}${formData.path || ""}`,
      {
    ...
    client.on("error", function (err) {
      alert(
        "There has some problems when create connection!\n Error is:" +
          err.message
      );
      client.end();
      client = null;
...
function handleRsuOptions(topic, msg, packet) {
  ...
  $("#bsmUpLimit").val(
    result.bsmConfig.upLimit === -1 ? "不限" : result.bsmConfig.upLimit
  );
  ...
  $("#rsmUpLimit").val(
    result.rsmConfig.upLimit === -1 ? "不限" : result.rsmConfig.upLimit
  );
  $("#rsmFilters").val(JSON.stringify(result.rsmConfig.upFilters));

  $("#mapUpLimit").val(
    result.mapConfig.upLimit === -1 ? "不限" : result.mapConfig.upLimit
  );
  $("#mapFilters").val(JSON.stringify(result.mapConfig.upFilters));

  $("#spatUpLimit").val(
    result.spatConfig.upLimit === -1 ? "不限" : result.spatConfig.upLimit
  );
...
function handleSubscribe(event) {
  ...
  if (client) {
    ...
    client.subscribe(topic, { qos: parseInt(qos) }, function (err) {
      if (err) {
        alert(
          `There has some problems when subscribe topic "${formData.topic}"!\nError:${err.message}`
        );
      } else {
...
function handlePublishDataset() {
  ...
  function syncPublish(data, virData, name) {
    ...
    const cooperData = virData
      .flat()
      .find((item) => item.name === msg_vir_name);

    if (time_rsm && msg_vir_name && cooperData) {
      const sameTimeData = cooperData?.data?.find(
        (item) => time_rsm === item.timeStamp
      );
      if (sameTimeData) {
...
function handleMessage(topic, msg, packet) {
  ...
  messageList.prepend(`<li style="border-left: solid 10px ${getColorForSubscription(
    topic
  )};">
...
function handleDataSetSendMessage(topic, msg) {
  ...
  sendMessageList.prepend(`<li style="border-left: solid 10px ${getColorForSubscription(
    topic
  )};">
                  <div class="container-fluid message">
...
function handleSubscribeSettingButton() {
  ...
  if (client) {
    ...
    client.subscribe(topic, { qos: 0 }, function (err) {
      if (err) {
        alert(
          `There has some problems when subscribe topic "${topic}"!\nError:${err.message}`
        );
      }
      TOPIC_COLOR_MAP[topic] = color;
      $("#topicList").append(
